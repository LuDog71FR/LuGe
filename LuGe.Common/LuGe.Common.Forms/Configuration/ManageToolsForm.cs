...
namespace LuGe.Common.Forms.Configuration
{
	...
	public partial class ManageToolsForm : Form
	{
		...
		private void ShowData(ToolInfo tool)
		{
			if (tool == null) return;
		...
		private void LoadSettings()
		{
			...
			foreach(ToolInfo tool in settings.Tools)
			{
		...
		{
			ToolInfoSettings settings = new ToolInfoSettings();
			
			settings.Tools.Clear();
			foreach(ToolInfo tool in listBoxTools.Items)
			{
				settings.Tools.Add(tool);
			}
			
			LIConfiguration<ToolInfoSettings>.Save(settings);			
		}
		...
		private void ListBoxToolsSelectedIndexChanged(object sender, EventArgs e)
		{
			...
			ShowData((ToolInfo)listBoxTools.SelectedItem);
		}
		...
		private void MoveTool(MoveDirection direction)
		{
			...
			ToolInfo tool = (ToolInfo)listBoxTools.SelectedItem;
			if (tool.Position + delta < 1) return;
			if (tool.Position + delta > listBoxTools.Items.Count) return;
			...
			for(int position=1; position <= listBoxTools.Items.Count; position++)
			{
				(listBoxTools.Items[position-1] as ToolInfo).Position = position;
			}
		}
		...
		private void SaveToTool(ToolInfo tool)
		{
		...
		private void ButtonAddClick(object sender, EventArgs e)
		{
			...
			tool.Position = listBoxTools.Items.Count + 1;
			SaveToTool(tool);
